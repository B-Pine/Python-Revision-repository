...
#
# The forest can be reached from the road, and the hill; so those should be the descriptions that appear in
# your list.
#
...
#
# Finally, wrap your comprehension in a for loop, and print the lists of all the locations that lead to
# each of the other locations in turn. In other words, use a for loop to run the comprehension for each of
# the keys in the locations dictionary.
...
for loc in range(0, 6):
    forest_entrance_comprehension = [(key, locations[key]) for key, value in exits.items() if loc in
                                     value.values()]
    print(forest_entrance_comprehension)

# forest_entrance = []
# for key, value in exits.items():
#     # print(key, value)
#     if loc in value.values():
#         forest_entrance.append((key, locations[key]))
#
# print(forest_entrance)
